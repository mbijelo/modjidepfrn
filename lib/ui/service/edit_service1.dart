...
class EditServiceScreen1 extends StatefulWidget {
  @override
  _EditServiceScreen1State createState() => _EditServiceScreen1State();
}

class _EditServiceScreen1State extends State<EditServiceScreen1> {
  ...
  _init(){
      ...
      _controllerPrice[index].text = "10";
      textFieldToEnd(_controllerPrice[index]);
      _controllerDiscPrice[index].text = "10";
      textFieldToEnd(_controllerDiscPrice[index]);
      _controllerNamePrice[index].text = "10";
      textFieldToEnd(_controllerNamePrice[index]);
      _currentPriceLevel = 0;
      productSetPrice("10", _currentPriceLevel);
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
        ...
        children: [
          ...
          new Opacity(opacity: 1.0, child: new Padding(
        ...
        appbar1(Colors.transparent, !_wait ? Colors.black : Colors.white ,
            strings.get(178), context, () {
          if (_openDialogAddons){
            _openDialogAddons = false;
            _redraw();
          }else
  ...
  _saveDataProduct() async {
    ...
    if (ret == null) {
      ...
      messageOk(context, strings.get(116));
    }/// "Data saved",
    else {
  ...
  var CNameArabic = "";
  var Lacategorie = "";
  var dejaPublieDansCetteCategorie = false;

  _body() {
    List<Widget> list = [];
    for(var catego in categories){
      if(!ifCategoryHaveSubcategories(catego.id)) {
        dejaPublieDansCetteCategorie = false;
        for (var Leservice in product) {
          if (Leservice.category[0] == catego.id) {
            dejaPublieDansCetteCategorie = true;
          }
        }

        if (!dejaPublieDansCetteCategorie){
          Lacategorie = getCategoryNameById(catego.id);
                                  ...
                                  if (text2.isNotEmpty)
                                    Row(children: [
                                      InkWell(
                                          onTap: () async {
                                            _waits(true);
                                            _editControllerName.text = "s_"+realText+"_"+currentProvider.name[0].text+"_"+currentProvider.id+"_"+DateTime.now().toString();
                                            List<StringData> x = [];
                                            x.add(new StringData());
                                            x[0].text=_editControllerName.text;
                                            currentProduct.name = x;

                                            _controllerDescTitle.text = "";
                                            _controllerDesc.text = "";
                                            _controllerTax.text = "10";
                                            _controllerDuration.text = "1";

                                            _controllerPrice[0].text = "10";
                                            _controllerDiscPrice[0].text = "10";
                                            _controllerNamePrice[0].text = "10";


                                            List<PriceData> xx = [];
                                            List<StringData> xx1 = [];
                                            xx1.add(new StringData());
                                            xx1[0].text="10";
                                            xx.add(new PriceData(xx1, 10,10,"Fixed",ImageData()));
                                            currentProduct.price = xx;


                                            List<String> LesCategories = [];
                                            LesCategories.add(catego.id);
                                            currentProduct.category = LesCategories;
                                          ...
                                          child: Container(
                                            ...
                                            decoration: BoxDecoration(
                                              ...
                                              borderRadius: BorderRadius
                                                  .circular(aTheme.radius),
                                            ),
                                            child: Text(strings.get(294),
